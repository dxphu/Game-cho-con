...
const SafeBacteria: React.FC<{ emoji: BacteriaType; size: number }> = ({ emoji, size }) => {
  const [isError, setIsError] = useState(false);
  return (
    <div style={{ width: size, height: size }} className="flex items-center justify-center">
      {!isError ? (
        <img 
          src={ASSETS.BACTERIA[emoji]} 
...
const Tooth: React.FC<ToothProps> = ({ stains, onClean, brushPos }) => {
  ...
  const bacteriaMap = useMemo(() => {
    ...
    stains.forEach(s => {
      map[s.id] = BACTERIA_KEYS[Math.floor(Math.random() * BACTERIA_KEYS.length)];
    });
    return map;
  }, [stains.length]);
  ...
  return (
    <div className="relative w-80 h-96 md:w-[480px] md:h-[580px] flex items-center justify-center">
      ...
      {stains.map(stain => !stain.isCleaned && (
        ...
        >
          {stain.type === 'bacteria' ? (
            <SafeBacteria emoji={bacteriaMap[stain.id]} size={stain.size} />
          ) : (
            <div 
